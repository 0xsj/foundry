...
func TestSnapshot_PayloadIsDeepCopied(t *testing.T) {
	...
	// Mutate original event's payload (via the store's internal slice)
	// The snapshot's payload must not change.
	// (We reach in through All() to get the actual stored event)
	all := s.All()
