...
func TestParseLogLine_PairsOnly(t *testing.T) {
	// Message is empty — only key=value pairs after service
...
func TestParseLogLine_PairsNotNilOnError(t *testing.T) {
	line := `2024-01-15T14:30:00Z INFO svc: no pairs here`
	entry, err := ParseLogLine(line)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Pairs must be a non-nil map — callers should be able to range over it
	if entry.Pairs == nil {
...
var batchLines = []string{
	...
	`not a log line`, // parse error — should be recorded, not fatal
...
func TestParseLogBatch_Counts(t *testing.T) {
	...
	// 5 valid lines (blank skipped, 1 parse error)
	...
	if len(result.ParseErrors) != 1 {
		t.Errorf("ParseErrors len = %d, want 1 (for %q)", len(result.ParseErrors), "not a log line")
	}
}
...
func TestParseLogBatch_BlankLinesSkipped(t *testing.T) {
	lines := []string{
		"",
		"   ",
		`2024-01-15T14:30:00Z INFO svc: msg`,
		"",
	}
