...
func TestUnixTimeMarshal(t *testing.T) {
	...
	// Must be an integer, not a quoted string
	if string(data)[0] == '"' {
...
func TestV1ResponseShape(t *testing.T) {
	...
	// created_at must be an integer (Unix timestamp), not a string
	...
	// DeletedAt must NOT appear in V1
...
func TestV2ResponseShape(t *testing.T) {
	...
	// created_at must be a string (RFC3339), not an integer
	if val, ok := raw["created_at"]; ok {
	...
	// api_version must be at top level (promoted from embedded Meta)
	checkStringField(t, raw, "api_version", "v2")

	// links must be at top level (promoted from embedded Meta)
	if _, ok := raw["links"]; !ok {
		t.Error("V2 response missing links field (should be promoted from embedded Meta)")
	}

	// deleted_at must be absent when nil
	if _, ok := raw["deleted_at"]; ok {
...
func TestV2MetaIsEmbedded(t *testing.T) {
	// If Meta were a named field "meta", this key would appear in JSON.
	// It must NOT appear — Meta fields must be promoted, not nested.
	resp := NewV2Response(activeProduct, baseURL)
	data, _ := json.Marshal(resp)
	var raw map[string]any
	json.Unmarshal(data, &raw)
	...
	if _, ok := raw["meta"]; ok {
		t.Error("V2 must not have a top-level 'meta' key — Meta must be embedded so its fields are promoted")
	}
}
...
func TestSerializeJSONV1(t *testing.T) {
	...
	// Check pretty-printing (should contain newlines)
	if !strings.Contains(string(data), "\n") {
		t.Error("SerializeJSON should return pretty-printed JSON (with newlines)")
	}
...
func TestSerializeJSONV2(t *testing.T) {
	...
	// First item (active): api_version present, no deleted_at
	...
	// Second item (deleted): deleted_at present
...
func TestSerializeCSV(t *testing.T) {
	s := NewSerializer(baseURL)
	products := []Product{activeProduct, deletedProduct, noTagProduct}
	data, err := s.SerializeCSV(products)
	if err != nil {
	...
	// deleted_at column should be empty for active product
	activeCols := strings.Split(active, ",")
	if len(activeCols) < 7 {
		t.Fatalf("row 1 should have 7 columns, got %d: %q", len(activeCols), active)
	}
	if activeCols[6] != "" {
	...
	if cols[4] != "" {
		t.Errorf("row 3 tags (no tags) should be empty string, got %q", cols[4])
	}
}
