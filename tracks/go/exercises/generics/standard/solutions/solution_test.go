...
func TestLen(t *testing.T) {
	...
	c.Delete(2)
	if c.Len() != 2 {
		t.Fatalf("after delete: Len=%d, want 2", c.Len())
	}
}
...
func TestTTLExpiration(t *testing.T) {
	...
	v, ok := c.Get("session")
	if !ok || v != "user-123" {
		t.Fatalf("expected hit immediately, got ok=%v v=%q", ok, v)
	}
...
func TestLRUAccessUpdatesOrder(t *testing.T) {
	...
	c.Set("b", 2)
	c.Set("c", 3)
	c.Get("a") // a becomes MRU; LRU order: b, c, a
...
func TestLRUSetUpdatesOrder(t *testing.T) {
	...
	c.Set("b", 2)
	c.Set("c", 3)
	c.Set("a", 99) // re-set "a" → moves to MRU; LRU: b, c, a
	c.Set("d", 4)  // should evict "b"
	v, aOK := c.Get("a")
	_, bOK := c.Get("b")
	if !aOK || v != 99 {
		t.Fatalf("'a' should be present with updated value, got ok=%v v=%d", aOK, v)
	}
	if bOK {
		t.Fatal("'b' should have been evicted")
	}
