...
func TestParse_DefaultTimeout(t *testing.T) {
	// Timeout not set — should default to 30
	input := "name=svc\nhost=localhost\nport=8080\n"
	cfg, err := Parse(input)
	if err != nil {
		t.Fatalf("Parse() error = %v, want nil", err)
...
func TestValidate_MissingMultipleFields(t *testing.T) {
	...
	// Should report all missing fields — check the error message contains each field
	msg := err.Error()
...
func TestValidate_ErrInvalidConfig(t *testing.T) {
	// Any validation failure should satisfy errors.Is(err, ErrInvalidConfig)
	cfg := Config{Timeout: 30} // missing name, host, port
	err := Validate(cfg)
...
func TestLoad_FileReadError(t *testing.T) {
	...
	// The original read error should be accessible via errors.Is
